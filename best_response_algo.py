...
import numpy as np
from constants import MAX_COUNT, MIN_Y, MAX_Y
...
def verify_equilibrium(X, equi_Y, true_Y, p, agents=None):
    ...
    for i in agents:
        if in_range(equi_Y[i], MIN_Y) and (proj[i]-eps > true_Y[i] or proj[i] + eps > true_Y[i]):
            continue
        elif in_range(equi_Y[i], MAX_Y) and (proj[i]-eps < true_Y[i] or proj[i] + eps < true_Y[i]):
            continue
        elif not in_range(equi_Y[i], MAX_Y) and not in_range(equi_Y[i], MIN_Y) and in_range(proj[i], true_Y[i]):
            continue
        else:
            return False
    return True
